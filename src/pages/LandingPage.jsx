...
const t = {
  en: {
    ...
    flow: {
      ...
      steps: [
        { n: '01', title: 'Upload', desc: 'Drop a PDF or TXT — it never leaves your browser unprotected.' },
        { n: '02', title: 'Anonymize', desc: 'PII is stripped locally using regex tuned for Moroccan documents.' },
        { n: '03', title: 'Hash & Prove', desc: 'SHA-256 fingerprint logged on-chain via smart contract.' },
        { n: '04', title: 'AI Insight', desc: 'Anonymized text analyzed by Llama 3 in multiple languages.' },
        { n: '05', title: 'Certify', desc: 'Download your blockchain-backed trust certificate.' },
      ],
    },
    trust: {
      label: 'Security',
  ...
  fr: {
    ...
    flow: {
      ...
      steps: [
        { n: '01', title: 'Importation', desc: 'Déposez un PDF ou TXT — il ne quitte jamais votre navigateur sans protection.' },
        { n: '02', title: 'Anonymisation', desc: 'Les données personnelles sont supprimées localement avec des regex adaptées aux documents marocains.' },
        { n: '03', title: 'Empreinte & preuve', desc: 'L\'empreinte SHA-256 est enregistrée on-chain via contrat intelligent.' },
        { n: '04', title: 'Analyse IA', desc: 'Le texte anonymisé est analysé par Llama 3 en plusieurs langues.' },
        { n: '05', title: 'Certification', desc: 'Téléchargez votre certificat de confiance ancré dans la blockchain.' },
      ],
    },
    trust: {
      label: 'Sécurité',
    ...
    footer: {
      tagline: 'Couche de confiance documentaire IA, axée sur la confidentialité.',
  ...
  ar: {
    ...
    flow: {
      ...
      steps: [
        { n: '01', title: 'رفع الملف', desc: 'أسقط ملف PDF أو TXT — لن يغادر متصفحك بدون حماية.' },
        { n: '02', title: 'إخفاء الهوية', desc: 'يتم حذف البيانات الشخصية محلياً باستخدام أنماط مخصصة للمستندات المغربية.' },
        { n: '03', title: 'البصمة والإثبات', desc: 'يتم تسجيل بصمة SHA-256 على البلوكشين عبر عقد ذكي.' },
        { n: '04', title: 'رؤية الذكاء الاصطناعي', desc: 'يحلل Llama 3 النص المجهول بلغات متعددة.' },
        { n: '05', title: 'الاعتماد', desc: 'حمّل شهادة ثقتك المرتكزة على البلوكشين.' },
      ],
    },
    trust: {
      label: 'الأمان',
...
function ServiceCard({ icon, title, desc, index }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 17, fontWeight: 600, color: '#e2e8f0', marginBottom: 10 }}>{title}</div>
      <div style={{ fontSize: 14, color: 'rgba(148,163,184,0.85)', lineHeight: 1.75 }}>{desc}</div>
    </div>
  );
}
  ...
  return (
    <div ref={ref} style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
      ...
      <div style={{ paddingBottom: isLast ? 0 : 32 }}>
        ...
        <div style={{ fontSize: 14, color: 'rgba(148,163,184,0.8)', lineHeight: 1.7 }}>{desc}</div>
      </div>
    </div>
  );
}
...
export default function LandingPage({ lang, setLang, onEnter, onCompanyMode }) {
  ...
  useEffect(() => {
    ...
    tl.fromTo(badgeRef.current, { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6 })
      .fromTo(h1Ref.current,   { y: 40,  opacity: 0 }, { y: 0, opacity: 1, duration: 0.8 }, '-=0.3')
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{
        position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none',
        background: 'radial-gradient(ellipse 80% 60% at 50% 0%, rgba(30,64,175,0.18) 0%, transparent 70%)',
      }} />
      ...
      {/* ── HERO ── */}
      <section ref={heroRef} style={{ ...styles.hero, position: 'relative', zIndex: 1 }}>
        {/* Grid overlay */}
        <div style={{
        ...
        <div ref={ctaRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, zIndex: 1 }}>
          <button
            onClick={onEnter}
            style={{
              padding: '16px 44px',
            ...
            onMouseEnter={e => {
              ...
              e.currentTarget.style.boxShadow = '0 16px 48px rgba(37,99,235,0.55)';
            }}
            onMouseLeave={e => {
              e.currentTarget.style.transform = 'translateY(0) scale(1)';
              e.currentTarget.style.boxShadow = '0 8px 40px rgba(37,99,235,0.4)';
            }}
          >
            {copy.hero.cta} →
          </button>
          <span style={{ fontSize: 12, color: 'rgba(148,163,184,0.5)', letterSpacing: '0.04em' }}>
            {copy.hero.ctaSub}
          </span>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, letterSpacing: '0.12em', color: '#94a3b8' }}>SCROLL</div>
          <div style={{
            width: 1, height: 40,
      ...
      <section style={{ ...styles.section, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          {copy.services.items.map((s, i) => (
            <ServiceCard key={i} index={i} icon={s.icon} title={s.title} desc={s.desc} />
          ))}
        </div>
      </section>
      ...
      <section style={{ ...styles.section, position: 'relative', zIndex: 1 }}>
        <div style={{
          ...
          <div style={{ position: 'sticky', top: 100 }}>
            ...
            <div style={{ ...styles.sectionTitle, marginBottom: 16 }}>{copy.flow.title}</div>
            <p style={{ fontSize: 14, color: 'rgba(148,163,184,0.7)', lineHeight: 1.8 }}>
              {lang === 'ar'
        ...
        <div style={{
          ...
          <div style={{
            ...
            {copy.trust.items.map((item, i) => (
              <TrustStat key={i} stat={item.stat} label={item.label} index={i} />
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: '3.5rem' }}>
            <button
              ...
              style={{
                padding: '14px 40px',
                borderRadius: 12, border: '1px solid rgba(96,165,250,0.35)',
                background: 'rgba(96,165,250,0.08)',
      ...
      <footer style={{
        ...
        <div style={{ fontSize: 13, color: 'rgba(148,163,184,0.6)', marginBottom: 6 }}>
          {copy.footer.tagline}
        </div>
...
function TrustStat({ stat, label, index }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 13, color: 'rgba(148,163,184,0.7)', lineHeight: 1.5 }}>{label}</div>
    </div>
  );
}
